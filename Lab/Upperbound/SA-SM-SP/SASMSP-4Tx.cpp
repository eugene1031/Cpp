...
#define  OUTPUTFILE  "sasmsp4Tx.txt"
#define  OUTPUTFILEA  "bbb.txt"
#define  OUTPUTFILED "sasmsp4txdebug.txt"
...
#define	 bits_num		30
#define  inter	        2
#define  codeword	    1073741824 // 2^30
...
void TxRx()
{
	...
	for (int a = 0; a < 1; a++) {  //for (int a = 0; a < codeword - 1; a++) {
		...
		for (int b = 1; b < 1073741824; b++) { //for (int b = a + 1; b < codeword; b++) { //for (int b = 1; b < codeword; b++) {
			...
			*/
			...
			//new define
			X_Q.col(0) << I_me.col(sp840[col_b][0] - 1);
			X_Q.col(1) << I_me.col(sp840[col_b][1] - 1);
			X_Q.col(2) << I_me.col(sp840[col_b][2] - 1);
			X_Q.col(3) << I_me.col(sp840[col_b][3] - 1);
			X_Q.col(4) << I_me.col(sp840[col_b][4] - 1);
			X_Q.col(5) << I_me.col(sp840[col_b][5] - 1);
			X_Q.col(6) << I_me.col(sp840[col_b][6] - 1);
			X_Q.col(7) << I_me.col(sp840[col_b][7] - 1);
